...
##### Source Text Files Migration

```bash
# Dry run (simulation without uploading)
npm run migrate:source-text:dry

# Perform actual migration
npm run migrate:source-text

# Advanced options
npx tsx scripts/migrateSourceTextFiles.ts --books=hamlet,the-republic --force
```
