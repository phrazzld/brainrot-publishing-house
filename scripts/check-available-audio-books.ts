...
async function checkBookAudio(bookSlug: string): Promise<{ bookSlug: string; hasAudio: boolean; audioCount: number }> {
...
async function main() {
  ...
  const results = await Promise.all(
    translations.map(book => checkBookAudio(book.slug))
  );
  ...
  console.log(`Total audio files across all books: ${booksWithAudio.reduce((sum, book) => sum + book.audioCount, 0)}`);
}
