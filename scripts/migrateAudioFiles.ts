...
import { blobService, blobPathService } from '../utils/services';
...
const EXTERNAL_AUDIO_BASE_URL = process.env.NEXT_PUBLIC_AUDIO_SOURCE_URL || 'https://brainrot-audio-assets.example.com';
...
class AudioFileMigrator {
  ...
  private async processBook(bookSlug: string): Promise<void> {
    ...
    for (const audioPath of audioPaths) {
      ...
      this.semaphore--;
      const promise = this.processAudioFile(audioPath, book.slug)
        .finally(() => {
          this.semaphore++;
  ...
  private async processAudioFile(audioPath: string, bookSlug: string): Promise<void> {
    ...
    if (!audioPath.startsWith('/') && !audioPath.includes('://') && !audioPath.startsWith('books/')) {
      blobPathInput = `books/${audioPath}`;
    }
    const blobPath = audioPath.includes('://') ? audioPath : blobPathService.convertLegacyPath(blobPathInput);
