 ...
 */
function identifyPlaceholders(files: ListBlobResultBlob[], sizeThreshold: number): PlaceholderFile[] {
  return files
  ...
  try {
    ...
    if (options.verbose || placeholders.length < 10) {
      ...
      placeholders.forEach((file, index) => {
        console.log(`${index + 1}. ${file.path} (${file.size} bytes, uploaded: ${file.uploaded.toISOString()})`);
      });
    }
