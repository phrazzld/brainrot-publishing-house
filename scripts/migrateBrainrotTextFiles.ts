#!/usr/bin/env node
/**
 * Brainrot Text Files Migration Script
 * 
 * This script migrates brainrot text files from the local filesystem to Vercel Blob storage.
 * It scans book directories for brainrot text files and uses the BlobService to upload them.
 * 
 * Usage:
...
/**
 * Brainrot Text Migration Service
 */
class BrainrotTextMigrationService {
  private migrationLog: MigrationLog;
  
  constructor(
    private readonly blobService = blobService,
    private readonly blobPathService = blobPathService,
    logFile: string = 'brainrot-text-migration.json'
  ) {
  ...
  /**
   * Find brainrot text files for a specific book
   */
  private async findBrainrotTextFiles(bookSlug: string): Promise<string[]> {
    const brainrotDir = path.join(ASSETS_DIR, bookSlug, 'text', 'brainrot');
    
    if (!existsSync(brainrotDir)) {
      return [];
    }
    
    const files = await fs.readdir(brainrotDir);
    return files.filter(file => file.toLowerCase().endsWith('.txt'));
  }
  
  /**
   * Migrate all brainrot text files
   */
  public async migrateAll(options: MigrationOptions = {}): Promise<MigrationResult> {
    ...
    console.log('Starting brainrot text files migration');
    ...
    for (const chunk of chunks) {
      await Promise.all(chunk.map(async (bookSlug) => {
        ...
        try {
          // Find all brainrot text files for this book
          const textFiles = await this.findBrainrotTextFiles(bookSlug);
          bookResult.total = textFiles.length;
          
          if (textFiles.length === 0) {
            console.log(`No brainrot text files found for book: ${bookSlug}`);
            result.books[bookSlug] = bookResult;
            return;
          }
          
          console.log(`Found ${textFiles.length} brainrot text files for book: ${bookSlug}`);
          ...
          for (const fileChunk of fileChunks) {
            await Promise.all(fileChunk.map(async (fileName) => {
              try {
                ...
                // Calculate paths
                const originalPath = `/assets/${bookSlug}/text/brainrot/${fileName}`;
                ...
                const failedResult: TextFileMigrationResult = {
                  status: 'failed',
                  originalPath: `/assets/${bookSlug}/text/brainrot/${fileName}`,
                  blobPath: this.blobPathService.convertLegacyPath(`/assets/${bookSlug}/text/brainrot/${fileName}`),
                  blobUrl: '',
                  error: errorMessage,
                };
...
function parseArgs(): MigrationOptions {
  const options: MigrationOptions = {
    ...
    retries: 3,
    concurrency: 5,
    logFile: 'brainrot-text-migration.json',
  };
...
async function main(): Promise<void> {
  try {
    ...
    const migrationService = new BrainrotTextMigrationService(
      blobService,
      blobPathService,
      options.logFile
    );
