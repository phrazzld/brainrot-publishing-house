...
class AudioFilesMigrator {
   ...
   */
  private async createPlaceholderMp3(filePath: string, sizeKb = 50): Promise<{ path: string, size: number }> {
  ...
  private releaseSemaphore(): void {
    ...
    if (this.uploadQueue.length > 0 && this.semaphoreValue > 0) {
      ...
      if (nextUpload) {
        this.activePromises.push(nextUpload().catch(error => {
  ...
  async run(): Promise<AudioMigrationResult[]> {
    ...
    try {
      ...
      console.log();
      console.log(`Total uploaded  : ${chalk.blue((totalUploadSize / (1024 * 1024)).toFixed(2) + ' MB')}`);
      console.log(`Total duration  : ${chalk.blue((totalDuration / 1000).toFixed(2) + ' seconds')}`);
      console.log();
  ...
  private async processBook(bookSlug: string): Promise<void> {
    ...
    console.log(chalk.blue(`📖 Processing book: ${chalk.white(book.title)} (${chalk.gray(book.slug)})`));
      ...
      if (this.semaphoreValue > 0) {
        ...
        this.activePromises.push(
          this.processAudioFile(chapter.audioSrc!, book.slug, chapter.title)
            .catch(error => console.error(`Error processing ${book.slug}/${chapter.title}:`, error))
            .finally(() => this.releaseSemaphore())
        );
      } else {
    ...
    try {
      ...
      try {
        ...
        if (fileInfo.size > 10 * 1024 && !this.options.force) {
          console.log(chalk.yellow(`   Skipping existing file: ${targetBlobPath} (${fileInfo.size} bytes)`));
        ...
        if (this.options.verbose) {
          console.log(chalk.gray(`   File not found in Blob storage, will upload: ${targetBlobPath}`));
        }
      }
      
      // Create a placeholder MP3 file with random size between 50-100KB
      const randomSize = Math.floor(Math.random() * 51) + 50; // 50-100KB
      const placeholderFile = await this.createPlaceholderMp3(
      ...
      if (this.options.dryRun) {
        console.log(chalk.gray(`   [DRY RUN] Would upload ${placeholderFile.path} (${placeholderFile.size} bytes) to ${targetBlobPath}`));
      ...
      const uploadResult = await blobService.uploadFile(fileBuffer, {
      ...
      if (this.options.verbose) {
        console.log(chalk.green(`   ✅ Uploaded: ${targetBlobPath} (${verifyResult.size} bytes) as ${verifyResult.contentType}`));
      }
...
function parseArgs(): MigrationOptions {
  const args = minimist(process.argv.slice(2), {
    ...
    string: ['books', 'log-file'],
    number: ['retries', 'concurrency'],
    default: {
      'dry-run': false,
      'books': '',
      'force': false,
