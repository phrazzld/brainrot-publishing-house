...
class AudioFilesMigrator {
   ...
   */
  private getBookAudioFiles(book: typeof translations[0]): Array<{ path: string; chapterTitle: string }> {
  ...
  private async processAudioFile(
    ...
    try {
      ...
      this.log(`   ✅ Downloaded ${downloadResult.size} bytes (${downloadResult.contentType}) in ${downloadDuration}ms`);
      ...
      this.log(`   ✅ Uploaded to ${uploadResult.url} (${uploadResult.size} bytes) in ${uploadDuration}ms`);
      
      // Verify the upload
      const verifyResult = await blobService.getFileInfo(uploadResult.url);
      if (verifyResult.size !== downloadResult.size) {
        throw new Error(`Upload verification failed: size mismatch (expected ${downloadResult.size}, got ${verifyResult.size})`);
      }
