    ...
    for (const file of files) {
      try {
        ...
        const result = await blobService.uploadFile(fileObj, {
          ...
          filename,
          access: 'public',
          cacheControl: 'max-age=31536000, immutable' // 1 year cache for images
        });
        
        console.log(`✅ Uploaded ${file.filename} to ${result.url}`);
      } catch (error) {
        console.error(`❌ Error uploading ${file.filename}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
