...
async function verifyAudioMigration(bookSlugs?: string[]) {
  ...
  const books = bookSlugs 
    ? translations.filter(t => bookSlugs.includes(t.slug))
    : translations;
  ...
  for (const book of books) {
    ...
    for (const chapter of chaptersWithAudio) {
      ...
      try {
        ...
        const isValidAudio = fileInfo.size > 100 * 1024 && 
        ...
        if (isValidAudio) {
          validAudioFiles++;
          console.log(`   ✅ Valid audio file: ${formatSize(fileInfo.size)}, ${fileInfo.contentType}`);
        } else {
          smallPlaceholders++;
          console.log(`   ⚠️ Small placeholder file: ${formatSize(fileInfo.size)}, ${fileInfo.contentType}`);
        }
  ...
  console.log(`Files found in Blob storage: ${existingFiles} (${formatPercentage(existingFiles, totalFiles)})`);
  console.log(`Valid audio files: ${validAudioFiles} (${formatPercentage(validAudioFiles, totalFiles)})`);
  console.log(`Small placeholder files: ${smallPlaceholders} (${formatPercentage(smallPlaceholders, totalFiles)})`);
...
// Run verification
verifyAudioMigration(bookSlugs.length > 0 ? bookSlugs : undefined)
  .catch(error => {
